...
export class PlayAreaTestComponent {
    ...
    async buttonDetect(event: KeyboardEvent) {
        ...
        if (this.buttonPressed === 'Enter') {
            ...
            this.handleQCMChoice(
                this.question.choices[index - 1].text,
                await this.questionService.checkAnswer([this.question.choices[index - 1].text], this.question.id),
            );
        }
    }
    ...
    async updateScore() {
        if (this.question.type === 'QCM') {
            const index = parseInt(this.buttonPressed, 10);
            if ((await this.questionService.checkAnswer([this.question.choices[index - 1].text], this.question.id)) && this.answer !== '') {
                this.score += this.question.points;
            }
        } else {
            this.score += this.question.points;
